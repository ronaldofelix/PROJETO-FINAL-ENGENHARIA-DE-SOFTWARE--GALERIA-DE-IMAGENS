...
function logout() {
    ...
    updateUIBasedOnLogin();
    showLoginModal(); // Show the login modal after logout
}
...
async function loadImages() {
    try {
        ...
        images.forEach(image => {
            addImageToGallery(`http://localhost:3000${image.path}`);
        });
    } catch (error) {
...
uploadForm.addEventListener('submit', async (e) => {
    ...
    const previewImages = previewContainer.querySelectorAll('.preview-item');
    ...
    try {
        ...
        for (const file of files) {
            ...
            const data = await response.json();
            addImageToGallery(`http://localhost:3000${data.filePath}`);
        }
...
async function deleteImage(imageDiv) {
    const img = imageDiv.querySelector('img');
...
function updateUIBasedOnLogin() {
    if (isLoggedIn) {
        ...
        document.getElementById('botao-abrir-galeria').style.display = 'none';
    } else {
        searchContainer.style.display = 'none';
        addDescriptionButton.style.display = 'none';
        document.getElementById('logout-button').style.display = 'none';
        document.getElementById('botao-abrir-galeria').style.display = 'block';
    }
}
...
document.getElementById('botao-abrir-galeria').addEventListener('click', showLoginModal);
